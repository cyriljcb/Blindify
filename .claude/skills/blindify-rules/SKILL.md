...
Un joker par joueur et par partie (rendu par `RejouerPartie`), utilisable uniquement pendant la phase de réponse d'un round classique — jamais en bonus, jamais après avoir répondu, jamais en pause, jamais une deuxième fois. Aucune pénalité, le chrono continue. `GameHub.UtiliserJoker(roundId)` (joueur uniquement) réduit le champ des possibles selon mode/cible (50/50 en Qcm, 4 tuiles sur 26 en PremiereLettre, structure du texte + pochette floutée ou décennie révélée en TapeReponse — jamais la réponse elle-même) et renvoie un `JokerIndiceDto` à l'appelant seul ; diffuse `JokerUtilise{playerId}` à tout le groupe sans rien révéler. L'indice est persisté sur le round (rejoué à l'identique en cas de reconnexion). Les réponses données avec joker (`RoundAnswer.AvecJoker`) sont exclues des statistiques de difficulté (`Reponses`/`Confusions` dans `stats.json`). Le titre `JOKER_GACHE` sanctionne qui a utilisé son joker et s'est quand même trompé. Détail complet (tableau mode/cible, contrat, endpoint de pochette) dans `docs/architecture.md` section 6.
